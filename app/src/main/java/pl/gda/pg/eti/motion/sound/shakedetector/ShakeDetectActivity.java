package pl.gda.pg.eti.motion.sound.shakedetector;
...
public class ShakeDetectActivity implements SensorEventListener {
     ...
     */
    private static final int IGNORE_EVENTS_AFTER_SHAKE = 10;
    ...
    public void checkForShake() {
        ...
        lastShakeValue = (float) 0.0;
        int x_pos =0, x_neg=0, x_dir = 0, y_pos=0, y_neg=0, y_dir=0, z_pos=0, z_neg = 0, z_dir = 0;
        for(DataPoint dp: dataPoints){
            if (dp.x > POSITIVE_COUNTER_THRESHOLD && x_dir < 1) {
                ++x_pos;
                x_dir = 1;
            }
            if (dp.x < NEGATIVE_COUNTER_THRESHOLD && x_dir > -1) {
                ++x_neg;
                x_dir = -1;
            }
            if (dp.y > POSITIVE_COUNTER_THRESHOLD && y_dir < 1) {
                ++y_pos;
                y_dir = 1;
            }
            if (dp.y < NEGATIVE_COUNTER_THRESHOLD && y_dir > -1) {
                ++y_neg;
                y_dir = -1;
            }
            if (dp.z > POSITIVE_COUNTER_THRESHOLD && z_dir < 1) {
                ++z_pos;
                z_dir = 1;
            }
            if (dp.z < NEGATIVE_COUNTER_THRESHOLD && z_dir > -1) {
                ++z_neg;
                z_dir = -1;
            }
...
//        Log.i("CHANGE", Integer.toString(x_pos) + " - " + Integer.toString(x_neg) + "  " + Integer.toString(y_pos) + " - " + Integer.toString(y_neg) + "  " + Integer.toString(z_pos) + " - " + Integer.toString(z_neg));
        ...
        if ((x_pos >= MINIMUM_EACH_DIRECTION && x_neg >= MINIMUM_EACH_DIRECTION) ||
            ...
            dataPoints.clear();
//            Log.i("CHANGE", "ITS A SHAKE! = " + maxAcc);
